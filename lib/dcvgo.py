...
class DirectContractedVoxGO(nn.Module):
    def __init__(self, xyz_min, xyz_max,
                 ...
                 density_config={}, k0_config={}, seg_config=seg_config_default,
                 rgbnet_dim=0,
        ...
        # init segmentation representation
        # feature voxel grid + shallow MLP  (fine stage)
        rgbnet_dim_raw = rgbnet_dim
        if len(seg_config) > 0:
            segnet_dim, s0_config, s0_type, segnet_width, segnet_depth, seg_class = \
                seg_config['segnet_dim'],seg_config['s0_config'],seg_config['s0_type'],\
                seg_config['segnet_width'],seg_config['segnet_depth'],seg_config['seg_class']
            rgb_and_s = seg_config.get("fuse_rgb_and_seg", True)
            self.rgb_and_s = rgb_and_s

            if rgb_and_s:
                segnet_dim = segnet_dim+rgbnet_dim
                rgbnet_dim = segnet_dim


            self.seg_kwargs = seg_config
            self.seg_class = seg_class
            self.s0_dim = segnet_dim
            self.s0_config = s0_config
            if not rgb_and_s:
                self.s0 = grid.create_grid(
                    s0_type, channels=self.s0_dim, world_size=self.world_size,
                    xyz_min=self.xyz_min, xyz_max=self.xyz_max,
                    config=self.s0_config)
            else:
                self.s0 = None
            ...
            self.segnet = nn.Sequential(
                ...
                *[
                    nn.Sequential(nn.Linear(segnet_width, segnet_width), nn.ReLU(inplace=True))
                    for _ in range(segnet_depth - 2)
                ],
                nn.Linear(segnet_width, seg_class),
            )
            nn.init.constant_(self.segnet[-1].bias, 0)
            print('dcvgo: segmentation feature voxel grid', self.s0)
            print('dcvgo: segmentation mlp', self.segnet)
        else:
            self.segnet = None
            self.s0 = None
    ...
    def scale_volume_grid(self, num_voxels):
        ...
        if self.s0 is not None:
            self.s0.scale_volume_grid(self.world_size)
    ...
    def k0_total_variation_add_grad(self, weight, dense_mode):
        ...
        # add seg
        if self.s0 is not None:
            self.s0.total_variation_add_grad(w,w,w,dense_mode)
    ...
    def sample_ray(self, ori_rays_o, ori_rays_d, stepsize, is_train=False, **render_kwargs):
        ...
        ray_deps = (rays_d[:,None,:] * t[None,:,None]).norm(dim=-1)[...,None]/ori_rays_d.norm(dim=-1)[...,None,None]\
                   *self.scene_radius## 不知道这里xyz的radius为什么是相同的
        ...
        return ray_pts, inner_mask.squeeze(-1), t, ray_deps
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, is_train=False, **render_kwargs):
        ...
        ray_pts, inner_mask, t, ray_deps = self.sample_ray(
        ...
        ray_deps = ray_deps[mask]  # add depth
        ...
        if render_kwargs.get('render_depth', False):
            depth = segment_coo(
                src=(weights * ray_deps[...,0]),
                index=ray_id,
                out=torch.zeros([N]),
                reduce='sum')

            depth += alphainv_last*1000  # the depth of sky of
        ...
        if self.segnet is not None:
            ...
            seg_marched = segment_coo(
                src=(weights.unsqueeze(-1) * seg),
                index=ray_id,
                out=torch.zeros([N, self.seg_class]),
                reduce='sum')
            # random background segmentation
            seg_marched += (alphainv_last.unsqueeze(-1) * torch.rand_like(seg_marched))
            ret_dict.update({'segmentation': seg_marched})
