...
def get_training_rays_flatten(rgb_tr_ori,train_poses, HW, Ks, ndc, inverse_y, flip_x, flip_y,
                              depth_tr_ori=None, seg_tr_ori=None):
    ...
    if depth_tr_ori is not None:
        ...
        seg_tr = torch.zeros([N, ], device=DEVICE)
        for c2w, img,depth,seg , (H, W), K in zip(train_poses, rgb_tr_ori,depth_tr_ori, seg_tr_ori, HW, Ks):
            ...
            depth_tr[top:top+n].copy_(depth.flatten(0,1))
            seg_tr[top:top + n].copy_(seg.flatten(0, 1))
            imsz.append(n)
            top += n
    else:
    ...
    if depth_tr_ori is not None:
        return rgb_tr,depth_tr, seg_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz
    return rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz


@torch.no_grad()
def get_training_rays_patch(rgb_tr_ori,train_poses, HW, Ks, ndc, inverse_y, flip_x, flip_y,
                              depth_tr_ori=None, seg_tr_ori=None, patch_size=[64,64]):
    print('get_training_rays_patch: start')
    hs, ws = patch_size
    assert len(rgb_tr_ori) == len(train_poses) and len(rgb_tr_ori) == len(Ks) and len(rgb_tr_ori) == len(HW)
    eps_time = time.time()
    DEVICE = rgb_tr_ori[0].device
    ...
    if depth_tr_ori is not None:
        ...
        for c2w, img,depth,seg , (H, W), K in zip(train_poses, rgb_tr_ori,depth_tr_ori, seg_tr_ori, HW, Ks):
            assert img.shape[:2] == (H, W)
            assert abs(H%hs)+abs(W%ws) == 0, 'Invaild patch size {},{}'.format(hs,ws)
            ...
            for i in range(H//hs):
                for j in range(W//ws):
                    n = hs*ws
                    rgb_tr[top:top + n].copy_(img[i*hs:((i+1)*hs),j*ws:((j+1)*ws)].flatten(0, 1))
                    rays_o_tr[top:top + n].copy_(rays_o[i*hs:((i+1)*hs),j*ws:((j+1)*ws)].flatten(0, 1).to(DEVICE))
                    rays_d_tr[top:top + n].copy_(rays_d[i*hs:((i+1)*hs),j*ws:((j+1)*ws)].flatten(0, 1).to(DEVICE))
                    viewdirs_tr[top:top + n].copy_(viewdirs[i*hs:((i+1)*hs),j*ws:((j+1)*ws)].flatten(0, 1).to(DEVICE))
                    depth_tr[top:top + n].copy_(depth[i*hs:((i+1)*hs),j*ws:((j+1)*ws)].flatten(0, 1))
                    seg_tr[top:top + n].copy_(seg[i*hs:((i+1)*hs),j*ws:((j+1)*ws)].flatten(0, 1))
                    top += n
        ...
        for c2w, img, (H, W), K in zip(train_poses, rgb_tr_ori, HW, Ks):
            ...
            for i in range(H // hs):
                for j in range(W // ws):
                    n = hs * ws
                    rgb_tr[top:top + n].copy_(img[i * hs:((i + 1) * hs), j * ws:((j + 1) * ws)].flatten(0, 1))
                    rays_o_tr[top:top + n].copy_(
                        rays_o[i * hs:((i + 1) * hs), j * ws:((j + 1) * ws)].flatten(0, 1).to(DEVICE))
                    rays_d_tr[top:top + n].copy_(
                        rays_d[i * hs:((i + 1) * hs), j * ws:((j + 1) * ws)].flatten(0, 1).to(DEVICE))
                    viewdirs_tr[top:top + n].copy_(
                        viewdirs[i * hs:((i + 1) * hs), j * ws:((j + 1) * ws)].flatten(0, 1).to(DEVICE))
                    top += n
    ...
    assert top == N
    eps_time = time.time() - eps_time
    print('get_training_rays_patch: finish (eps time:', eps_time, 'sec)')
...
def batch_indices_generator_patch(N, patch_size):
    # torch.randperm on cuda produce incorrect results in my machine
    ...
    while True:
        if top + 1 > n:
            idx, top = torch.LongTensor(np.random.permutation(n)), 0
        yield torch.LongTensor(np.array(list(range(idx[top]*bs,(idx[top]+1)*bs))))
        top += 1

def batch_indices_generator_mix(N, patch_size, n_rand=4096):
    bs = (patch_size[0] * patch_size[1])
    N_p = N//bs
    ...
    while True:
        ...
        if top + rand_batch > N:
            idx, top = torch.LongTensor(np.random.permutation(N)), 0
        patch = torch.cat([torch.LongTensor(list(range(idx_p[top_p+i] * bs, (idx_p[top_p+i]+1) * bs)))
                           for i in range(patch_batch)], dim=0)
        # patch = torch.LongTensor(np.array(list(range(idx_p[top_p] * bs, (idx_p[top_p]+1) * bs))))
        rand = idx[top:top+rand_batch]
