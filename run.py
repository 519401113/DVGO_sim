...
@torch.no_grad()
def render_viewpoints_with_seg(model, render_poses, HW, Ks, ndc, render_kwargs,
                      gt_imgs=None, savedir=None, dump_images=False,
                      render_factor=0, render_video_flipy=False, render_video_rot90=0,
                      eval_ssim=False, eval_lpips_alex=False, eval_lpips_vgg=False):
    '''Render images for the given viewpoints; run evaluation if gt given.
...
def load_everything(args, cfg):
    ...
    if 'segmentations' in data_dict.keys():
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    def gather_training_rays(depth=False):
        ...
        if depth:
            if data_dict['irregular_shape']:
                depth_tr_ori = [depths[i].to('cpu' if cfg.data.load2gpu_on_the_fly else device) for i in i_train]
                seg_tr_ori = [segs[i].to('cpu' if cfg.data.load2gpu_on_the_fly else device) for i in i_train]
            else:
                depth_tr_ori = depths[i_train].to('cpu' if cfg.data.load2gpu_on_the_fly else device)
                seg_tr_ori = segs[i_train].to('cpu' if cfg.data.load2gpu_on_the_fly else device)
        if cfg_train.ray_sampler == 'in_maskcache':
            ...
            result = dvgo.get_training_rays_flatten(
                rgb_tr_ori=rgb_tr_ori,
                train_poses=poses[i_train],
            ...
            result = dvgo.get_training_rays_patch(
                ...
                HW=HW[i_train], Ks=Ks[i_train], ndc=cfg.data.ndc, inverse_y=cfg.data.inverse_y,
                flip_x=cfg.data.flip_x, flip_y=cfg.data.flip_y,
                depth_tr_ori=depth_tr_ori,seg_tr_ori=seg_tr_ori, patch_size=patch_size)
            if depth_tr_ori is not None:
                rgb_tr, depth_tr, seg_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz = result
            else:
                rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz = result
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        if cfg_train.ray_sampler in ['flatten', 'in_maskcache','patch','mixed','one_img_mixed']:
            ...
            # save_image(target.reshape(4*patch_size[0],patch_size[1],-1).permute(2, 1, 0).cpu(), 'test_patch.png')
        ...
        if cfg_train.depth_loss>0:
            ...
            # add smoothness loss
            if cfg_train.ray_sampler in ['patch','mixed']:
                length = len(render_result['depth']) if cfg_train.ray_sampler == 'patch' \
...
if __name__=='__main__':
    ...
    if args.render_train:
        ...
        # imageio.mimwrite(os.path.join(testsavedir, 'video.depth.mp4'), utils.to8b(1 - depths / np.max(depths)), fps=1, quality=8)

        from vis import *
        logits_2_label = lambda x: torch.argmax(x, dim=-1)
        from torchvision.utils import save_image

        os.makedirs(os.path.join(testsavedir, 'rgbs'), exist_ok=True)
        os.makedirs(os.path.join(testsavedir, 'depths'), exist_ok=True)
        for i in range(rgbs.shape[0]):
            depth_i = np.clip(depths[i],0,depths[i][depths[i]<200].mean()*5)
            # vis semantic
            if len(segs) > 0:
                seg = torch.Tensor(segs[i])
                seg = logits_2_label(seg)
                visualize_semantic(seg.cpu(), pathname=testsavedir, idx=i)
            save_image(torch.tensor((1 - depth_i / np.max(depth_i))).permute(2,0,1), os.path.join(testsavedir,'depths','{:04d}.png'.format(i)))
            save_image(torch.tensor(rgbs[i]).permute(2,0,1), os.path.join(testsavedir,'rgbs', '{:04d}.png'.format(i)))
